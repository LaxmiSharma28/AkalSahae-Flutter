...
import 'dart:math';
import 'package:akalsahae/app/helper/body/payload_model.dart';
import 'package:akalsahae/app/modules/Order_Confirm_Module/OrderConfirmScreen.dart';
...
class NotificationHelper {
  ...
  static Future<void> showNotification(RemoteMessage message, FlutterLocalNotificationsPlugin fln, bool data) async {
    ...
    if (data) {
      ...
      _image = (message.data['image'] != null && message.data['image'].isNotEmpty) ? message.data['image'] : null;
    } else {
      ...
      if (GetPlatform.isAndroid) {
        _image =
            (message.notification!.android!.imageUrl != null && message.notification!.android!.imageUrl!.isNotEmpty)
  ...
  static Future<void> showBigTextNotification(
      String title, String body, String payload, String image, FlutterLocalNotificationsPlugin fln) async {
...
/*  static final FlutterLocalNotificationsPlugin notificationsPlugin = FlutterLocalNotificationsPlugin();
  ...
  static Future<void> showNotificationForeground(RemoteMessage message) async {
    final notificationDetails = NotificationDetails(
        android: AndroidNotificationDetails("com.example.push_notification", "push_notification",
            importance: Importance.max, priority: Priority.high));

    notificationsPlugin.show(
        DateTime.now().microsecond, message.notification!.title, message.notification!.body, notificationDetails,
        payload: message.data.toString());

    FirebaseMessaging.onMessage.listen((event) {
      showNotificationForeground(event);
      Get.to(() => OrderConfirmScreenPage);
    });
  ...
  static Future<void> initilize() async {
    ...
    await notificationsPlugin.initialize(initializationSettings, onSelectNotification: (event) {
      print("DATA CHECK ==>>> ${jsonEncode(event)}");
    });
  }*/
